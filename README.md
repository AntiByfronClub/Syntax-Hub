...
local F = {
    Aim = { On=false, Held=false, Part="Head", TeamCheck=true, WallCheck=false, Smooth=0.20, FOV=120, ShowFOV=true, Predict=0.0, Trigger=false, TrigDelay=60, Silent=false, Fire=false },
    ESP = { On=false, Box=true, Filled=false, Name=true, Dist=true, Health=true, Tracer=false, Skeleton=false, EnemiesOnly=false },
    Mov = { Speed=false, SpeedV=50, JumpOn=false, JumpV=80, Fly=false, FlyV=2.5, Noclip=false, InfJump=false, CarFly=false, CarFlySpeed=120, Spin=false, SpinV=5, NcGlitch=false },
    Wld = { Fullbright=false, NoFog=false, FOV=70, AntiAFK=false },
    UI  = { Boxes=true, RGB=false },
    Esc = { AutoEscape=false },

    KA  = { On=false, Radius=10, Sphere=false, TeamCheck=false, Type="All" },
    AA  = { On=false, Radius=8, TeamCheck=false, Type="Both" },
...
local PlayerTasedEvent = nil

pcall(function()
    local gr=RS:FindFirstChild("GunRemotes") or RS:WaitForChild("GunRemotes",5)
    ShootRemote = gr and (gr:FindFirstChild("ShootEvent") or gr:WaitForChild("ShootEvent",5))
    ReloadRemote = ShootRemote and ShootRemote.Parent:FindFirstChild("FuncReload")
    PlayerTasedEvent = gr and gr:FindFirstChild("PlayerTased")
end)

-- ─── SILENT AIM ───────────────────────────────────────────────────────────
local _silentHook = false
local _silentOld = nil

local function setupSilentAim()
    if _silentOld then return end
    local hookmeta = hookmetamethod
    local getnc    = getnamecallmethod
    local ischeck  = checkcaller
            ...
            if F.Aim.Silent and attached and self == ShootRemote
               and (not ischeck or not ischeck()) and method == "FireServer" then
                ...
                if type(shots) == "table" then
                    local ok, t = pcall(function() return Aim.getTarget(F.Aim.FOV, F.Aim.WallCheck) end)
                    if ok and t then
    ...
    if hookfunction then
        ...
        pcall(function()
            ...
            if gr then
                ...
                if se then
                    local conns2 = getconnections and getconnections(se.OnServerEvent)
                    if conns2 and conns2[1] then
                        plBullet = conns2[1].Function
                    end
                end
            end
        end)

        if plBullet and not _silentOld then
            _silentOld = hookfunction(plBullet, function(origin, direction, ...)
                if not F.Aim.Silent then return _silentOld(origin, direction, ...) end
                local t = Aim.getTarget(F.Aim.FOV, F.Aim.WallCheck)
                if t then
                    return _silentOld(origin, t.Position, ...)
                end
                return _silentOld(origin, direction, ...)
            end)
            _silentHook = true
        end
    end
...
do
    ...
    function Aim.getTarget(maxFov, needLos)
        local best,bd=nil,maxFov or F.Aim.FOV; local center=aimOrigin()
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=LocalPlayer and isEnemy(p) then
                local ok,c=alive(p); local part=ok and c:FindFirstChild(F.Aim.Part)
                if part then
...
-- ─── KILL AURA SPHERE VISUAL ──────────────────────────────────────────────
local kaSphere=Instance.new("Part")
kaSphere.Shape=Enum.PartType.Ball; kaSphere.Anchored=true; kaSphere.CanCollide=false
kaSphere.Material=Enum.Material.ForceField; kaSphere.Color=Color3.fromRGB(255,0,50)
kaSphere.Transparency=1; kaSphere.CastShadow=false; kaSphere.CanQuery=false; kaSphere.CanTouch=false
kaSphere.Parent=Workspace

local KA_TYPES={"All","Criminals","Inmates","Guards","Criminals + Inmates","Criminals + Guards","Inmates + Guards","Other Teams"}
local function updateKAAllowed()
    local lt=LocalPlayer.Team and LocalPlayer.Team.Name or ""
    local t=F.KA.Type; local allowed={}
    if t=="All" then allowed={"Criminals","Inmates","Guards"}
    elseif t=="Criminals" then allowed={"Criminals"}
    elseif t=="Inmates" then allowed={"Inmates"}
    elseif t=="Guards" then allowed={"Guards"}
    elseif t=="Criminals + Inmates" then allowed={"Criminals","Inmates"}
    elseif t=="Criminals + Guards" then allowed={"Criminals","Guards"}
    elseif t=="Inmates + Guards" then allowed={"Inmates","Guards"}
    elseif t=="Other Teams" then
        if lt=="Inmates" then allowed={"Criminals","Guards"}
        elseif lt=="Criminals" then allowed={"Inmates","Guards"}
        else allowed={"Criminals","Inmates"} end
    end
    F.KA._allowed=allowed
end
updateKAAllowed()
F.KA._allowed=F.KA._allowed or {"Criminals","Inmates","Guards"}

local meleeEvent
pcall(function() meleeEvent=RS:FindFirstChild("meleeEvent") or RS:WaitForChild("meleeEvent",3) end)

-- ─── KILL AURA TARGET CHECK ───────────────────────────────────────────────
local function isKillable(p)
    if p == LocalPlayer then return false end
    local c = p.Character
    if not c then return false end
    if LocalPlayer.Team and LocalPlayer.Team.Name == "Guards"
       and p.Team and p.Team.Name == "Inmates" then
        if not c:GetAttribute("Hostile") then return false end
    end
    if table.find(ka_wl, p.Name) then return false end
    if not F.KA.TeamCheck then return true end
    local tn = p.Team and p.Team.Name or ""
    for _, n in ipairs(F.KA._allowed) do
        if tn == n then return true end
    end
    return false
end

-- ─── AUTO ARREST ──────────────────────────────────────────────────────────
local AA_TYPES={"Criminals","Inmates","Both"}
local _arrestCooldown = 0

local function isArrestable(p)
    if p == LocalPlayer then return false end
    if not (LocalPlayer.Team and LocalPlayer.Team.Name == "Guards") then return false end
    if p.Team and p.Team.Name == "Guards" then return false end
    local c = p.Character
    if not c then return false end
    if c:GetAttribute("Arrested") then return false end
    if p.Team and p.Team.Name == "Inmates" then
        if not (c:GetAttribute("Trespassing") or c:GetAttribute("Hostile")) then return false end
        if c:GetAttribute("Hostile") and not c:GetAttribute("Tased") then
        elseif not c:GetAttribute("Trespassing") then return false end
    end
    if table.find(aa_wl, p.Name) then return false end
    if not F.AA.TeamCheck then return true end
    local tn = p.Team and p.Team.Name or ""
    if F.AA.Type == "Criminals" then return tn == "Criminals"
    elseif F.AA.Type == "Inmates" then return tn == "Inmates"
    else return true end
end

local _aaThread = nil
local function startArrestLoop()
    if _aaThread then return end
    _aaThread = task.spawn(function()
        while F.AA.On and attached do
            if _arrestCooldown < os.clock() then
                local r = root()
                if r and ArrestRemote then
                    for _, p in ipairs(Players:GetPlayers()) do
                        if isArrestable(p) then
                            ...
                            if ph and phum and phum.Health > 0
                               and (r.Position - ph.Position).Magnitude <= F.AA.Radius then
                                local ok, result = pcall(function()
                                    return ArrestRemote:InvokeServer(p, 1)
                                end)
                                if ok and result then
                                    _arrestCooldown = os.clock() + 7
                                    Notify("Arrested " .. p.Name, 4)
                                    break
                                end
                            end
                        end
...
local _antiTazeEnabled = false

local function setupAntiTaze()
    if not PlayerTasedEvent then
        pcall(function()
            local gr = RS:FindFirstChild("GunRemotes") or RS:WaitForChild("GunRemotes", 5)
            PlayerTasedEvent = gr and gr:FindFirstChild("PlayerTased")
        end)
    end
    if not PlayerTasedEvent then return end
    if not getconnections then return end

    local function hookTaze()
        local conns2 = getconnections(PlayerTasedEvent.OnClientEvent)
        local conn = conns2 and conns2[1]
        if not (conn and conn.Function) then
            local deadline = os.clock() + 5
            while os.clock() < deadline and _antiTazeEnabled do
                conns2 = getconnections(PlayerTasedEvent.OnClientEvent)
                conn = conns2 and conns2[1]
                if conn and conn.Function then break end
                task.wait()
            end
        end
        if conn and conn.Function and _antiTazeEnabled then
            _antiTazeConn = conn
...
local function teardownAntiTaze()
    _antiTazeEnabled = false
...
bind(LocalPlayer.CharacterAdded,function(newChar)
    ...
    if _antiTazeEnabled then
        teardownAntiTaze()
        _antiTazeEnabled = true
        task.wait(0.3)
        setupAntiTaze()
    end
...
make("TextLabel",{Size=UDim2.fromOffset(160,34),Position=UDim2.fromOffset(78,1),BackgroundTransparency=1,Font=FONT,Text="·  v7.0  <3",TextColor3=T.sub,TextXAlignment=Enum.TextXAlignment.Left,TextSize=12},titleBar)
...
local function dropdown(p,labelText,getOptions,getCurrent,onSet)
    ...
    local btn=make("TextButton",{Size=UDim2.new(0.58,0,0,22),Position=UDim2.new(0.42,4,0.5,-11),BackgroundColor3=T.item,BorderSizePixel=0,AutoButtonColor=false,Font=FONT,Text=getCurrent().."   v",TextColor3=T.accent,TextSize=11},r); corner(btn,4); stroke(btn,T.line,1)
    reg(btn,"TextColor3")
    local function lbl2() btn.Text=getCurrent().."   v" end
...
make("TextLabel",{Size=UDim2.new(1,-24,1,0),Position=UDim2.fromOffset(32,0),BackgroundTransparency=1,Font=FONT,Text=LocalPlayer.Name.."   ·   Prison Life   ·   v7.0",TextColor3=T.sub,TextXAlignment=Enum.TextXAlignment.Left,TextSize=12},footer)
...
do
    ...
    local detItems={
        ...
        {text="Kill Aura  --  Detected (patched)",col=Color3.fromRGB(235,80,80),warn="    meleeEvent may be nil in this server"},
        {text="Arrest Aura  --  Detected (patched)",col=Color3.fromRGB(235,80,80)},
...
do
    ...
    toggle(sA,"Silent aim",function() return F.Aim.Silent end,function(v) F.Aim.Silent=v end)
    toggle(sA,"Triggerbot",function() return F.Aim.Trigger end,function(v) F.Aim.Trigger=v end)
    ...
    slider(sT,"Trigger delay (ms)",0,300,function() return F.Aim.TrigDelay end,function(v) F.Aim.TrigDelay=v end)
end
...
do
    ...
    local sKA=section(pg.left,"Kill Aura")
    label(sKA,"meleeEvent required — may be patched in some servers")
    toggle(sKA,"Enabled",function() return F.KA.On end,function(v) F.KA.On=v end)
    toggle(sKA,"Sphere visual",function() return F.KA.Sphere end,function(v) F.KA.Sphere=v end)
    toggle(sKA,"Team check",function() return F.KA.TeamCheck end,function(v) F.KA.TeamCheck=v end)
    dropdown(sKA,"Target type",function() return KA_TYPES end,function() return F.KA.Type end,function(opt) F.KA.Type=opt; updateKAAllowed() end)
    slider(sKA,"Radius",1,30,function() return F.KA.Radius end,function(v) F.KA.Radius=v end)
    button(sKA,"Add/Remove KA whitelist",function()
        if plTarget then
            local idx=table.find(ka_wl,plTarget.Name)
            if idx then table.remove(ka_wl,idx); Notify(plTarget.Name.." removed from KA whitelist")
            else table.insert(ka_wl,plTarget.Name); Notify(plTarget.Name.." added to KA whitelist") end
        end
    end)

    local sAA=section(pg.right,"Arrest Aura")
    label(sAA,"Guard team only — 7s cooldown between arrests")
    toggle(sAA,"Enabled",function() return F.AA.On end,function(v)
        F.AA.On=v
        if v then startArrestLoop() end
    end)
    toggle(sAA,"Team check",function() return F.AA.TeamCheck end,function(v) F.AA.TeamCheck=v end)
    dropdown(sAA,"Target type",function() return AA_TYPES end,function() return F.AA.Type end,function(opt) F.AA.Type=opt end)
    slider(sAA,"Radius",1,12,function() return F.AA.Radius end,function(v) F.AA.Radius=v end)
    button(sAA,"Add/Remove AA whitelist",function()
        if plTarget then
            local idx=table.find(aa_wl,plTarget.Name)
            if idx then table.remove(aa_wl,idx); Notify(plTarget.Name.." removed from AA whitelist")
            else table.insert(aa_wl,plTarget.Name); Notify(plTarget.Name.." added to AA whitelist") end
        end
    end)
end
...
do
    ...
    toggle(sF,"Noclip",function() return F.Mov.Noclip end,function(v) F.Mov.Noclip=v; if v then applyNoclip() else stopNoclip() end end)
...
do
    ...
    toggle(sP,"Anti-Tase (hook PlayerTased event)",function() return F.Prot.AntiTase end,function(v)
        F.Prot.AntiTase=v
        if v then
            _antiTazeEnabled=true
            setupAntiTaze()
    ...
    toggle(sP,"Anti-Void (teleport up if below map)",function() return F.Prot.AntiVoid end,function(v) F.Prot.AntiVoid=v end)
    toggle(sP,"Keep reset button enabled",function() return F.Prot.EnableReset end,function(v) F.Prot.EnableReset=v end)

    local sF=section(pg.right,"Feed")
...
do
    ...
    label(sI,"Kill Aura:  needs meleeEvent (patched in some servers)")
    label(sI,"Anti-Tase:  hooks PlayerTased event via getconnections")
...
bind(RunService.Heartbeat,function()
    ...
    if F.KA.On and meleeEvent then
        local r=root()
        if r then
            kaSphere.Size=Vector3.new(F.KA.Radius*2,F.KA.Radius*2,F.KA.Radius*2)
            kaSphere.Transparency=F.KA.Sphere and 0.6 or 1
            kaSphere.Position=r.Position
            local touching=Workspace:GetPartBoundsInRadius(r.Position,F.KA.Radius)
            local hit={}
            for _,part in ipairs(touching) do
                local model=part.Parent; local hh=model and model:FindFirstChild("Humanoid")
                if hh then
                    local tp=Players:GetPlayerFromCharacter(model)
                    if tp and not hit[tp] and isKillable(tp) then
                        hit[tp]=true
                        pcall(function() meleeEvent:FireServer(tp, 1, 1) end)
                    end
                end
            end
        end
    else kaSphere.Transparency=1 end
...
function Hub._unload()
    ...
    stopCarFly(); stopSpin()
    if healthConn then pcall(function() healthConn:Disconnect() end); healthConn=nil end
    if _aaThread then _aaThread=nil end
    teardownAntiTaze()
    ...
    pcall(function() kaSphere:Destroy() end)
...
if F.Prot.AntiTase then
    _antiTazeEnabled=true
    task.delay(2, setupAntiTaze)
end

setupAutoEscape()
if F.Auto.Keycard then startKeycard() end

warn("[syntax] v7.0 loaded -- Right Shift toggles UI · T = click-TP · Combat->RAGE to kill · KA/AA/AntiTaze ported from vape v4"
